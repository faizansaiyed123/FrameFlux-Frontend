        ...
        <section className="flex items-center justify-center px-5 py-10 sm:px-8">
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button type="submit" className="h-11 w-full cursor-pointer rounded-lg" disabled={loading}>{loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Creating account...</> : 'Create account'}</Button>
            </form>
            <p className="mt-7 text-center text-sm text-zinc-500 dark:text-zinc-400">Already have an account? <Link href="/auth/login" className="cursor-pointer font-medium text-zinc-950 hover:underline dark:text-white">Sign in</Link></p>
            <p className="mt-10 text-center text-[11px] leading-5 text-zinc-400 dark:text-zinc-500">By creating an account, you agree to our <Link href="/terms" className="cursor-pointer hover:text-zinc-600 hover:underline dark:hover:text-zinc-300">Terms of Service</Link> and <Link href="/privacy" className="cursor-pointer hover:text-zinc-600 hover:underline dark:hover:text-zinc-300">Privacy Policy</Link>.</p>
          </div>
        </section>
      </div>
    </main>
