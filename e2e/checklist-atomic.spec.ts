...
async function exercise(section:string, feature:string, ctx:Ctx) {
  ...
  if (section.includes('VIDEO → VIDEO') && VIDEO_FORMATS.includes(fmt||'')) {
    ...
    await ok(await ctx.request.post(API+`/media/${m.id}/convert`,{headers:{Authorization:'Bearer '+a.token},data:{format:fmt,width:160,height:120,fps:24,quality:5,video_codec:'h264',audio_codec:'aac'}}),f);
  ...
  if (l.includes('favorite')) { const a=await auth(ctx.request,'atomic-fav'),m=await uploadMedia(ctx.request,a.token,'e2e/fixtures/sample.mp4'); await ok(await ctx.request.post(API+`/favorites/${m.id}`,{headers:{Authorization:'Bearer '+a.token}}),f); return; }
